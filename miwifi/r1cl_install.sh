...
clear
echo "#############################################################"
echo "#             Install Shadowsocks for Miwifi(R1CL)          #"
echo "#############################################################"

cd /tmp
rm -f shadowsocks_r1cl.tar.gz
wget http://d.ukoi.net/Miwifi/R1CL/shadowsocks_r1cl.tar.gz
tar zxf shadowsocks_r1cl.tar.gz

# install shadowsocks ss-redir to /data/usr/sbin
mkdir -p /data/usr/sbin
cp -f ./shadowsocks_r1cl/ss-redir  /data/usr/sbin/ss-redir
chmod +x /data/usr/sbin/ss-redir

# Config shadowsocks init script
cp ./shadowsocks_r1cl/shadowsocks /etc/init.d/shadowsocks
...
#config dnsmasq
cp -f ./shadowsocks_r1cl/gfwlist.conf /etc/dnsmasq.d/gfwlist.conf
...
rm -rf /tmp/shadowsocks_r1cl
rm -f /tmp/shadowsocks_r1cl.tar.gz
echo ""
echo "Success install to R1CL"
echo ""
exit 0
